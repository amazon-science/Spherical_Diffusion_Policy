...
from sdp.model.common.normalizer import SingleFieldLinearNormalizer
from sdp.common.pytorch_util import dict_apply, dict_apply_reduce, dict_apply_split
import numpy as np
...
def get_torch_range_symmetric_normalizer_from_stat(stat, output_max=1, output_min=-1, range_eps=1e-7):
    # -1, 1 normalization
    input_max = stat['max']
    input_min = stat['min']
    input_range = input_max - input_min
...
def to_torch_stats(arr):
    arr = torch.from_numpy(arr)
    stat = {
        'min': arr.min(axis=0)[0],
        'max': arr.max(axis=0)[0],
