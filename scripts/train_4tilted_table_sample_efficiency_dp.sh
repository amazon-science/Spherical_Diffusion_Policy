...
do
  ...
  nohup python train.py --config-name=train_diffusion_unet \
  logging.project=${project} \
  task_name=${task[$i]} training.device=${g} \
  n_demo=${n_demo} \
  training.seed=${seed} \
  logging.name=$(date '+%m%d_%H%M')_dp_${task[$i]}_${n_demo}_${seed} \
  > ./logs/$(date '+%m%d_%H%M')_dp_${task[$i]}_${seed}_train.txt 2>&1 & sleep 10m
  ((g+=1))
done
